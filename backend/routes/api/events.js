...
//find events that user is attending by userId
router.get("/attending/users/:userId", async (req, res, next) => {
  let user;
  try {
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const newEvent = await Event.findOneAndUpdate(filter, update, {
        new: true,
      });
