		...
		<div id='event-modal'>
			...
			<div className='event-modal-image-group'>
				{event?.images?.map((image) => (
					<img src={image} className='event-modal-image'></img>
				))}
			</div>
            ...
            min={0}
            max={event?.guestNumber -  event?.guestLists.length}></input>
				</label>
				<button onClick={handleClick} className='settings-button'>Join the group!</button>
			</div>
		</div>
	);
}
